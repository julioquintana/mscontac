...
public class ContactServiceImpl implements ContactService {
	...
	public ContactResponse update(Long id, ContactRequest contactRequest) throws ValidationException {
		...
		if(!contactEntity.isPresent()) {
			throw new ValidationException("4030","Persona con el id: "+ id + " no existe en la bd",HttpStatus.NOT_FOUND);
		}
	...
	Optional<Contact> contact = dao.findById(id);
		...
		if(!contact.isPresent()) {
			throw new ValidationException("4030","Contact con el id: "+ id + " no existe en la bd",HttpStatus.NO_CONTENT);
		}
	...
	@Override
	public ContactResponse findAllContact()  throws ValidationException {
		
	Iterable<Contact> contact = dao.findAll();
	
		if(IterableUtils.size(contact) < 1) {
			throw new ValidationException("4030","La lista de contactos esta vacia",HttpStatus.NO_CONTENT);
		}
	...
	private ContactResponse buildResponseIterable(Iterable<Contact> iterable) {
		return mapper.map(iterable, ContactResponse.class);
	}	
}
